<h1 align="center">Insight-api-dinero</h1>

<div align="center">
  <strong>A Dinero blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/dinerocoin/insight-api-dinero">
    <img src="https://img.shields.io/travis/dinerocoin/insight-api-dinero/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-dinero">
    <img src="https://img.shields.io/npm/v/insight-api-dinero.svg?style=flat-square" alt="NPM version" />
  </a>
</div>
...
- [Resources](#resources)
- [License](https://github.com/dinerocoin/insight-api-dinero/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g bitcore-node-dinero@latest
bitcore-node-dinero create mynode
cd mynode
bitcore-node-dinero install insight-api-dinero
bitcore-node-dinero start
```

The API endpoints will be available by default at: `http://localhost:3001/insight-api-dinero/`

### Prerequisites

- [Bitcore Node Dinero 3.x](https://github.com/dinerocoin/bitcore-node-dinero)

**Note:** You can use an existing Dinero data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `dinero.conf`, as well as a few other additional fields.
...
### Block
```
  /insight-api-dinero/block/[:hash]
  /insight-api-dinero/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-dinero/block-index/[:height]
  /insight-api-dinero/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-dinero/rawblock/[:blockHash]
```
...
```
  /insight-api-dinero/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-dinero/tx/[:txid]
  /insight-api-dinero/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-dinero/rawtx/[:rawid]
  /insight-api-dinero/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-dinero/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-dinero/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-dinero/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-dinero/addr/[:addr]/balance
  /insight-api-dinero/addr/[:addr]/totalReceived
  /insight-api-dinero/addr/[:addr]/totalSent
  /insight-api-dinero/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-dinero/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-dinero/addrs/[:addrs]/utxo
  /insight-api-dinero/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-dinero/addrs/utxo
```
...
```
  /insight-api-dinero/txs/?block=HASH
  /insight-api-dinero/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-dinero/txs/?address=ADDR
  /insight-api-dinero/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-dinero/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-dinero/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-dinero/addrs/txs
```
...
GET method:
```
  /insight-api-dinero/gobject/list/proposal
```
...
GET method:
```
  /insight-api-dinero/gobject/get/[:hash]
  /insight-api-dinero/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-api-dinero/masternodes/list
```
### Validate Masternode
```
  /insight-api-dinero/masternodes/validate/[:payee]
  /insight-api-dinero/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-dinero/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-dinero/peer
```

### Status of the Bitcoin Network
```
  /insight-api-dinero/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-dinero/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a Dinero Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
